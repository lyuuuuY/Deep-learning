...
def build_DNN(input_shape, n_hidden_layers, n_hidden_units, loss,kl_weight=None, act_fun='sigmoid', optimizer:str='sgd', learning_rate=0.01,
    ...
    if use_variational_layer:
        for i in range(n_hidden_layers):
            model.add(tfp.layers.DenseVariational(n_hidden_units, activation=act_fun,make_prior_fn=prior,
                      make_posterior_fn=posterior,kl_weight= kl_weight ))
            if use_bn:
                model.add(BatchNormalization())
            if use_dropout:
                model.add(Dropout(0.5))
...
def train_DNN(config, training_config):
    ...
    history = model.fit(X_train,y_train, batch_size=training_config["batch_size"], epochs=training_config["epochs"],
                        validation_data=(X_val,y_val),class_weight=training_config["class_weight"],callbacks=[TuneReporterCallback()])

    final_accuracy = history.history["accuracy"][-1]
    final_loss = history.history["loss"][-1]
    return {"mean_accuracy": final_accuracy, "mean_loss": final_loss, "history":history,"model":model}
